<div class="main">
	<div class="container mt-5">
		<h2>Community Details</h2>
		<div class="row">
			<div class="col-md-4">
				<div class="card">
					<div class="card-body">
						<div class="card-image">
							<div class="pt-3 image-thumbnail">
								<img src="https://esportsnesia.com/wp-content/uploads/2018/06/evos-esports.jpg" alt="" style="width: 125px" class="img">
							</div>
						</div>
						<div class="mt-3 text-center">
							<h5 class="card-title text-center font-weight-bold">
								Dhaniel
							</h5>
							<div class="card-text text-secondary">
								Users
							</div>
						</div>
					</div>
				</div>
			</div>
			<div class="col-md">
				<div class="card">
					<div class="card-body">
						<h5 class="p-2">info</h5>
						<hr>
						<div class="container mt-3">
							<table class="table table-borderless text-white mx-auto">
								<tbody>
									<tr>
										<th>Name</th>
										<td class="text-secondary">Dhaniel</td>
									</tr>
									<tr>
										<th>Email</th>
										<td class="text-secondary">1@2.ocm</td>
									</tr>
									<tr>
										<th>Nickname</th>
										<td class="text-secondary">dhaniel</td>
									</tr>
									<tr>
										<th>Country</th>
										<td class="text-secondary">Indonesia</td>
									</tr>
								</tbody>
							</table>
						</div>
					</div>
				</div>
			</div>
		</div>
		<div class="row mt-4">
			<div class="col-md-4">
				
			</div>
			<div class="col-md">
				<div class="card">
					<div class="card-body">
						<h5 class="p-2">About</h5>
						<hr>
						<div class="container mt-3">
							<p class="text-secondary">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Nisi perferendis aut cupiditate quia corporis recusandae eligendi, voluptas vero amet odit ipsum a, aperiam maiores excepturi voluptatibus fugit provident assumenda ullam!</p>
						</div>
					</div>
				</div>
			</div>
		</div>
	</div>
</div>
<script src="<?= base_url() ?>assets/Admin/js/jquery.js"></script>
	<script src="<?= base_url() ?>assets/Admin/js/bootstrap.js"></script>
    <script type="text/javascript" src="<?php echo base_url() ?>assets/js/jquery.min.js"></script>
	<script type="text/javascript" src="<?php echo base_url() ?>assets/js/bootstrap.js"></script>
	<script type="text/javascript" src="<?php echo base_url() ?>assets/plugins/datatable/js/jquery.dataTables.js"></script>
	<script type="text/javascript" src="<?php echo base_url() ?>assets/plugins/datatable/js/dataTables.bootstrap.js"></script>
	<script src="https://cdn.jsdelivr.net/npm/bs-custom-file-input/dist/bs-custom-file-input.js"></script>
	<script src="https://cdn.jsdelivr.net/npm/sweetalert2@9"></script>
	<script src="<?= base_url() ?>assets/Admin/js/myscript.js"></script>